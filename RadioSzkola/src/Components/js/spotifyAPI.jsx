...
import '/src/Components/css/spotifyAPI.css';

const fetchData = async (updateLfmData) => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_HOST}/spotify`, {
      headers: {
        'Content-Type': 'application/json',
      }
    });
    updateLfmData(response.data);
  } catch (error) {
...
export const LastFmData = ({isLogged, userLoggedID}) => {
  ...
  const handleTogglePlayer = () => {
    document.getElementById('PlaybackContainer').classList.toggle('PlaybackContainerHidden');
    setIsContainerHidden((prevValue) => !prevValue);
  };
  
  useEffect(() => {
    fetchData(updateLfmData);
  }, []);
  ...
  const buildLastFmData = () => {
    ...
    const handleNextSong = () => {
      axios.post(import.meta.env.REACT_APP_HOST + 'tracks/nextsong', { id: userLoggedID })
        .then((res) => {
          setNotifications((prev) => [...prev, { message: "Aby pominąć utwór, potrzebne jest jeszcze " + res.data.userCount + " głosów.", type: 'success' }]);
        })
        .catch((err) => {
          if(err.response.status === 400) {
            setNotifications((prev) => [...prev, { message: "Coś poszło nie tak. Spróbuj ponownie później", type: 'error' }]);
          } else if (err.response.status === 429) {
            setNotifications((prev) => [...prev, { message: "Nie możesz wysyłać kolejnych głosów. Spróbuj ponownie później", type: 'error' }]);
          }
          console.error(err);
        });
    };
    ...
    const handlePreviousSong = () => {
      axios.post(import.meta.env.REACT_APP_HOST + 'tracks/previoussong', { id: userLoggedID })
        .then((res) => {
          setNotifications((prev) => [...prev, { message: "Aby pominąć utwór, potrzebne jest jeszcze " + res.data.userCount + " głosów.", type: 'success' }]);
        })
        .catch((err) => {
    ...
    return (
        <div className="PlaybackPositioningContainer">
          <div className="PlaybackContainer PlaybackContainerHidden" id="PlaybackContainer">
            <button className="TogglePlaybackContainer" onClick={handleTogglePlayer}>
              {!isContainerHidden ? (
            ...
            <div className="PlaybackItemsContainer">
                <img className="PlaybackAlbumCover" src={track.image[3]['#text']} alt="Album Cover" />
                <div className="PlaybackTrackContainer">
                  <h1 className="PlaybackTrackName">{songName}</h1>
                  <p className="PlaybackArtistName">{artistName}</p>
                </div>
                {isLogged && (
                  <div className="PlaybackControlsContainer">
                    <div className="PlaybackControlsPreviousSong">
                      <svg width="30" height="30" style={{cursor:"pointer"}} viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handlePreviousSong}>
                    ...
                    <div className="PlaybackControlsNextSong">
                      <svg width="29" height="30" style={{cursor:"pointer"}} viewBox="0 0 29 30" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handleNextSong}>
              ...
              <div className="PlaybackItemsContainer">
              <div className="PlaybackNothingPlaysCover"></div>
              <div className="PlaybackTrackContainer">
                <h1 className="PlaybackNothingPlaysText">Nic nie jest aktualnie odtwarzane</h1>
              </div>
          </div>
            )}
        </div>
